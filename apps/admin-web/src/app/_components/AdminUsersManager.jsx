...
export default function AdminUsersManager() {
  ...
  async function getAdmins() {
    try {
      setLoading(true)
      const res = await coreApi().get('/admin')
      setAdmins(res.data || [])
    } catch (error) {
  ...
  async function getTeams() {
    try {
      const res = await coreApi().get('/admin/teams')
  ...
  const onSubmitEdit = async (e, data) => {
    ...
    setSubmitting(true)
    try {
      await coreApi().put(`/admin/${editing.id}`, {
  ...
  const onSubmitCreate = async (e, data) => {
    ...
    setSubmitting(true)
    try {
      const res = await coreApi().post('/admin', {
  ...
  const onConfirmDelete = async () => {
    setSubmitting(true)
    try {
      await coreApi().delete(`/admin/${deleting.id}`)
