...
import { LIMIT, getTimeFormat, getValidDateFormat, photoUrl } from '@cocarr/shared-utils'
...
export default function Customers() {
  ...
  const getUsers = async () => {
    ...
    try {
      ...
      const res = await coreApi().get(`/admin/user-verification?${query}`)
  ...
  const createUser = async (e, data) => {
    e.preventDefault()
    try {
      await coreApi().post('/admin/user', data)
