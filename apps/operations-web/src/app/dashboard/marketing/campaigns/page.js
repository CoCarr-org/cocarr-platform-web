...
export default function Campaigns() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await coreApi().get('/admin/campaigns')
  ...
  useEffect(() => {
    load()
    coreApi().get('/admin/campaigns/segments')
    ...
    coreApi().get('/city').then((r) => setCities(r.data?.data || r.data || [])).catch(() => {})
  }, [])
  ...
  const runPreview = async () => {
    setPreviewing(true)
    try {
      const res = await coreApi().post('/admin/campaigns/audience-preview', {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing.id) await coreApi().put(`/admin/campaigns/${editing.id}`, payload)
      else await coreApi().post('/admin/campaigns', payload)
  ...
  const send = async (campaign) => {
    ...
    try {
      await coreApi().post(`/admin/campaigns/${campaign.id}/send`)
  ...
  const remove = async (campaign) => {
    ...
    try {
      await coreApi().delete(`/admin/campaigns/${campaign.id}`)
