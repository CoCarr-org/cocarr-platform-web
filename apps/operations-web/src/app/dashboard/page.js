...
export default function DashboardPage() {
  ...
  // The old team/level grid is gone; roles come from the IAM payload. The
  // dashboard BLOCKS are still composed server-side by core-api, which is the
  // point of this screen — it renders what it is given and filters nothing.
  const nav = useNavigation()
  const ready = nav.ready
  const level = null
  const source = null
  const team = nav.roles.length ? { name: nav.roles[0].key } : null
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await coreApi().get('/admin/dashboard/summary')
      ...
      <div className='mb-6 flex items-start justify-between gap-4 flex-wrap'>
        <div>
          <h1 className='text-xl font-semibold text-[#252525]'>
            {teamName ? `${teamName} Operations` : 'Operations'}
          </h1>
