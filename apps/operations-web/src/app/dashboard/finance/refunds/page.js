...
export default function RefundRequests() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await coreApi().get('/admin/refunds', {
  ...
  const openDetail = async (id) => {
    setBusy(`open-${id}`)
    try {
      const res = await coreApi().get(`/admin/refunds/${id}`)
      setOpen(res.data)
      setEdits({})
    } catch (error) {
  ...
  const save = async (approve) => {
    setBusy('save')
    try {
      await coreApi().put(`/admin/refunds/${open.id}`, { ...edits, approve })
      InfoToast(approve ? 'Refund approved' : 'Saved')
      const res = await coreApi().get(`/admin/refunds/${open.id}`)
  ...
  const process = async () => {
    ...
    setBusy('process')
    try {
      const res = await coreApi().post(`/admin/refunds/${open.id}/process`)
  ...
  const reject = async () => {
    ...
    setBusy('reject')
    try {
      await coreApi().post(`/admin/refunds/${open.id}/reject`, { reason: why.trim() })
  ...
  const runJob = async (dryRun) => {
    setBusy('build')
    try {
      const res = await coreApi().post('/admin/refunds/build', { dryRun })
      const r = res.data
      InfoToast(dryRun
