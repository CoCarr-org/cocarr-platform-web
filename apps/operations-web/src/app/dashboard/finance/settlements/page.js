...
export default function Settlements() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await coreApi().get('/admin/settlements', {
  ...
  const dryRun = async () => {
    setBusy('dry')
    try {
      const res = await coreApi().post('/admin/settlements/run', { dryRun: true })
      setPreview(res.data)
    } catch (error) {
  ...
  const runNow = async () => {
    ...
    setBusy('run')
    try {
      const res = await coreApi().post('/admin/settlements/run', { dryRun: false })
  ...
  const retry = async (row) => {
    ...
    setBusy(row.id)
    try {
      await coreApi().post(`/admin/settlements/${row.id}/retry`)
