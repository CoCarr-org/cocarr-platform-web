...
import FaceCompare from '@/app/_components/FaceCompare'
...
export default function UserDetailPage() {
  ...
  const { can: mayI } = usePermissions()
  const mayDecide = mayI('users', 'update')
  ...
  const load = async () => {
    ...
    try {
      const [profile, referrals, wallet] = await Promise.allSettled([
        authAxios.get(`/admin/user-verification/${id}`),
        authAxios.get(`/admin/wallet/user/${id}/referrals`),
        authAxios.get(`/admin/wallet/user/${id}`),
      ])
    ...
    try {
      const rr = await authAxios.get(`/admin/user-referrals/${id}`)
      setReferral(rr.data)
  ...
  const approve = () => {
    ...
    act('approve', async () => {
      await authAxios.post(`/admin/user-verification/${id}/approve`)
  ...
  const submitReject = () => {
    ...
    act('reject', async () => {
      await authAxios.post(`/admin/user-verification/${id}/reject`, { reason: reason.trim() })
  ...
  const submitSuspend = () => {
    ...
    act('suspend', async () => {
      await authAxios.post(`/admin/user-verification/${id}/suspend`, { suspended: true, reason: reason.trim() })
  ...
  const reactivate = () => {
    ...
    act('reactivate', async () => {
      await authAxios.post(`/admin/user-verification/${id}/suspend`, { suspended: false })
  ...
  const recheck = (type) => act(`re-check ${type}`, async () => {
    const res = await authAxios.post(`/admin/user-verification/${id}/recheck/${type}`)
  ...
  const setDocument = (docType, verified) => {
    if (!verified) {
      ...
      return act(`reject ${docType}`, async () => {
        await authAxios.post(`/admin/user-verification/${id}/document/${docType}`,
    ...
    return act(`verify ${docType}`, async () => {
      const res = await authAxios.post(`/admin/user-verification/${id}/document/${docType}`, { verified: true })
  ...
  const reRunOcr = (kind) => act(`re-read ${kind}`, async () => {
    const res = await authAxios.post(`/admin/user-verification/${id}/ocr/${kind}`)
