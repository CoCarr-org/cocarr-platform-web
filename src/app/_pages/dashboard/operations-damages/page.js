...
export default function DamageClaims() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await authAxios.get('/admin/damages', {
  ...
  const verify = async (claim, accept) => {
    ...
    setBusy(claim.id)
    try {
      await authAxios.post(`/admin/damages/${claim.id}/verify`, { accept, reason: reason?.trim() })
  ...
  const assess = async (claim) => {
    ...
    setBusy(claim.id)
    try {
      await authAxios.post(`/admin/damages/${claim.id}/assess`, {
  ...
  const openReview = async (claim) => {
    ...
    try {
      const res = await authAxios.get(`/admin/damages/${claim.id}`)
