...
export default function SupportCenter() {
  ...
  async function load() {
    try {
      ...
      const res = await authAxios.get(`/admin/tickets?${q}`)
  ...
  const onCreate = async (e, values) => {
    ...
    setSubmitting(true)
    try {
      await authAxios.post('/admin/tickets', values)
...
const TicketThread = ({ id, onClose, onChanged }) => {
  ...
  async function load() {
    try {
      const res = await authAxios.get(`/admin/tickets/${id}`)
      setData(res.data)
    } catch (error) {
  ...
  const send = async () => {
    ...
    setBusy(true)
    try {
      await authAxios.post(`/admin/tickets/${id}/messages`, { message: reply, isInternalNote: internal })
      setReply('')
      await load()
      onChanged?.()
    } catch (error) {
  ...
  const setField = async (field, value) => {
    setBusy(true)
    try {
      await authAxios.put(`/admin/tickets/${id}`, { [field]: value })
      await load()
      onChanged?.()
