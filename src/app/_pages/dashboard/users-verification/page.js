...
export default function UsersVerification() {
  ...
  const mayDecide = can('users', 'update')
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await authAxios.get('/admin/user-verification', {
  ...
  const submitRejection = async () => {
    ...
    try {
      await authAxios.post(`/admin/user-verification/${rejecting.id}/reject`, { reason: text })
  ...
  const submitSuspension = async () => {
    ...
    try {
      await authAxios.post(`/admin/user-verification/${suspending.id}/suspend`, { suspended: true, reason: text })
  ...
  const reactivate = async (u) => {
    ...
    setBusy(u.id)
    try {
      await authAxios.post(`/admin/user-verification/${u.id}/suspend`, { suspended: false })
  ...
  const setDoc = async (u, docKey, verified) => {
    ...
    setBusy(u.id)
    try {
      await authAxios.post(`/admin/user-verification/${u.id}/document/${api}`,
  ...
  const toggleSearchable = async (u) => {
    setBusy(u.id)
    try {
      await authAxios.post(`/admin/user-verification/${u.id}/searchable`, { searchable: !u.isSearchable })
