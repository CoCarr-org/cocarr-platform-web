...
export default function ReferralCampaigns() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await authAxios.get('/admin/referral-campaigns')
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing.id) await authAxios.put(`/admin/referral-campaigns/${editing.id}`, body)
      else await authAxios.post('/admin/referral-campaigns', body)
